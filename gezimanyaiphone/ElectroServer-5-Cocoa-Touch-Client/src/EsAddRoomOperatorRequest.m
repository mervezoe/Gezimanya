...
@implementation EsAddRoomOperatorRequest
...
- (id) initWithThriftObject: (id) thriftObject {
	if ((self = [super init])) {
		self.messageType = EsMessageType_AddRoomOperatorRequest;
		if (thriftObject != nil) {
...
- (void) fromThrift: (id) thriftObject {
	ThriftAddRoomOperatorRequest* t = (ThriftAddRoomOperatorRequest*) thriftObject;
	if ([t zoneIdIsSet]) {
...
- (ThriftAddRoomOperatorRequest*) toThrift {
	ThriftAddRoomOperatorRequest* _t = [[[ThriftAddRoomOperatorRequest alloc] init] autorelease];;
	if (zoneId_set_) {
		int32_t _zoneId;
		_zoneId = self.zoneId;
		_t.zoneId = _zoneId;
...
- (id) newThrift {
	return [[ThriftAddRoomOperatorRequest alloc] init];
}
