...
@implementation EsZoneListEntry
...
- (void) fromThrift: (id) thriftObject {
	ThriftZoneListEntry* t = (ThriftZoneListEntry*) thriftObject;
	if ([t zoneIdIsSet]) {
...
- (ThriftZoneListEntry*) toThrift {
	ThriftZoneListEntry* _t = [[[ThriftZoneListEntry alloc] init] autorelease];;
	if (zoneId_set_) {
		int32_t _zoneId;
		_zoneId = self.zoneId;
		_t.zoneId = _zoneId;
...
- (id) newThrift {
	return [[ThriftZoneListEntry alloc] init];
}
