...
@implementation EsEvictUserFromRoomRequest
...
- (id) initWithThriftObject: (id) thriftObject {
	if ((self = [super init])) {
		self.messageType = EsMessageType_EvictUserFromRoomRequest;
		if (thriftObject != nil) {
...
- (void) fromThrift: (id) thriftObject {
	ThriftEvictUserFromRoomRequest* t = (ThriftEvictUserFromRoomRequest*) thriftObject;
	if ([t zoneIdIsSet]) {
...
- (ThriftEvictUserFromRoomRequest*) toThrift {
	ThriftEvictUserFromRoomRequest* _t = [[[ThriftEvictUserFromRoomRequest alloc] init] autorelease];;
	if (zoneId_set_) {
		int32_t _zoneId;
		_zoneId = self.zoneId;
		_t.zoneId = _zoneId;
...
- (id) newThrift {
	return [[ThriftEvictUserFromRoomRequest alloc] init];
}
